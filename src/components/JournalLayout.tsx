...
import { Home, Book, Gamepad2, Youtube, Terminal, ArrowDownUp } from 'lucide-react';
...
export default function JournalLayout({ children, activeTab, onTabChange, sortOrder, onSortChange }: JournalLayoutProps) {
        ...
        <header className="sticky top-0 z-40 flex flex-col gap-4 border-b border-[rgb(var(--border))] bg-[rgb(var(--bg-primary))]/80 px-4 py-4 backdrop-blur-md md:flex-row md:items-center md:justify-between md:px-12">
          <div className="flex items-center justify-between md:justify-start gap-2">
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold tracking-tight font-display md:text-xl">Notes & Interests</h1>
              <span className="rounded-full bg-[rgb(var(--accent))]/10 px-2 py-0.5 text-xs font-mono text-[rgb(var(--accent))]">v2.1</span>
            </div>
            ...
            <button
              ...
              title="Sort"
            >
              <ArrowDownUp className={`h-4 w-4 transition-transform ${sortOrder === 'oldest' ? 'rotate-180' : ''}`} />
            </button>
          </div>
