...
export default function JournalGrid({ items }: { items: (Note | Book | Game | Channel)[] }) {
  const [selectedNote, setSelectedNote] = useState<Note | null>(null);

  const handleItemClick = (item: Note | Book | Game | Channel) => {
    if ('takes' in item) { // Check if it's a Note
                ...
                {'readTime' in item ? (
                  ...
                  <div className="group relative overflow-hidden rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-secondary))]/30 p-6 transition-all hover:border-[rgb(var(--accent))]/50 hover:shadow-lg hover:shadow-[rgb(var(--accent))]/5">
                    <div className="flex items-start justify-between">
                      ...
                      {item.link && <ArrowUpRight className="h-4 w-4 text-[rgb(var(--text-secondary))] opacity-0 transition-opacity group-hover:opacity-100" />}
                    </div>
                    <h3 className="mt-4 text-lg font-bold text-[rgb(var(--text-primary))]">{item.title}</h3>
                    <p className="text-sm text-[rgb(var(--text-secondary))]">{item.author}</p>
                    <div className="mt-4 flex items-center gap-2">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        item.status === 'Reading' ? 'bg-emerald-500/10 text-emerald-500' : 
                        item.status === 'Read' ? 'bg-blue-500/10 text-blue-500' : 
                  ...
                  <div className="group relative overflow-hidden rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-secondary))]/30 p-6 transition-all hover:border-[rgb(var(--accent))]/50 hover:shadow-lg hover:shadow-[rgb(var(--accent))]/5">
                    ...
                    <h3 className="mt-4 text-lg font-bold text-[rgb(var(--text-primary))]">{item.title}</h3>
                    <p className="text-sm text-[rgb(var(--text-secondary))]">{item.platform}</p>
                    <div className="mt-4 flex items-center gap-2">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        item.status === 'Playing' ? 'bg-purple-500/10 text-purple-500' : 
                  ...
                  <div className="group relative overflow-hidden rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-secondary))]/30 p-6 transition-all hover:border-[rgb(var(--accent))]/50 hover:shadow-lg hover:shadow-[rgb(var(--accent))]/5">
                    ...
                    <h3 className="mt-4 text-lg font-bold text-[rgb(var(--text-primary))]">{item.name}</h3>
                    <p className="text-sm text-[rgb(var(--text-secondary))]">{item.topic}</p>
                  </div>
                )}
              </motion.div>
            );
