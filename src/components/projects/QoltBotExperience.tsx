...
import { Database, Brain, MessageSquare, Search, FileText, Server, Cpu, Cloud, CheckCircle2, Play } from 'lucide-react';

const PIPELINE_STEPS = [
  { id: 'query', label: 'User Query', icon: MessageSquare, color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-400/20' },
  { id: 'embedding', label: 'Embedding', icon: Cpu, color: 'text-purple-400', bg: 'bg-purple-400/10', border: 'border-purple-400/20' },
  { id: 'search', label: 'Vector Search', icon: Search, color: 'text-yellow-400', bg: 'bg-yellow-400/10', border: 'border-yellow-400/20' },
  { id: 'context', label: 'Context Retrieval', icon: Database, color: 'text-green-400', bg: 'bg-green-400/10', border: 'border-green-400/20' },
  { id: 'generation', label: 'LLM Generation', icon: Brain, color: 'text-pink-400', bg: 'bg-pink-400/10', border: 'border-pink-400/20' },
  { id: 'response', label: 'Final Response', icon: MessageSquare, color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-400/20' },
];

const DEMO_QUERY = "What is the QoLT lab's mission?";
const DEMO_RESPONSE = "The Quality of Life Technology (QoLT) Laboratory's mission is to explore technological advancements and design intelligent systems for personalized health care, vital signs monitoring, and disease prevention.";
          ...
          <div className="mb-12 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-[rgb(var(--text-secondary))] mt-2 max-w-xl">
                Watch how QoltBot processes your question using Retrieval Augmented Generation.
              </p>
            </div>
            <button
                ...
                <div key={step.id} className="relative">
                  ...
                  <motion.div
                    animate={{
                      ...
                      opacity: index > activeStep && activeStep !== -1 ? 0.5 : 1,
                      borderColor: isActive || isCompleted ? 'rgba(var(--accent), 0.5)' : 'rgba(var(--border), 1)'
                    }}
                    className={`relative z-10 flex flex-col items-center gap-4 rounded-xl border p-4 text-center transition-colors duration-300 ${
                      isActive ? 'bg-[rgb(var(--bg-secondary))] shadow-xl ring-1 ring-[rgb(var(--accent))]/50' : 'bg-[rgb(var(--bg-primary))]'
                    }`}
                  >
                    <div className={`rounded-full p-3 ${step.bg} ${step.color} ring-1 ring-inset ${step.border}`}>
            ...
            <AnimatePresence mode="wait">
              {activeStep === 0 && (
                ...
                >
                  ...
                  <p className="text-lg font-medium text-[rgb(var(--text-primary))]">"{DEMO_QUERY}"</p>
                </motion.div>
              )}
              {activeStep === 1 && (
                <motion.div
                  ...
                  className="space-y-2"
                >
                  <p className="text-sm font-medium text-[rgb(var(--text-secondary))] uppercase tracking-wider">Generating Embeddings...</p>
                  <div className="font-mono text-xs text-[rgb(var(--accent))] break-all">
                    [0.023, -0.145, 0.892, 0.331, -0.567, 0.112, ... 384 dimensions]
                  </div>
                </motion.div>
              )}
              {activeStep === 2 && (
                ...
                >
                  ...
                  <p className="text-[rgb(var(--text-primary))]">Searching ChromaDB for nearest neighbors...</p>
                </motion.div>
              )}
              {activeStep === 3 && (
                <motion.div
                  ...
                  className="space-y-3"
                >
                  <p className="text-sm font-medium text-[rgb(var(--text-secondary))] uppercase tracking-wider">Retrieved Context</p>
                  <div className="rounded bg-[rgb(var(--bg-secondary))] p-3 text-sm text-[rgb(var(--text-secondary))] border-l-4 border-green-500 italic">
                    "...Quality of Life Technology Laboratory’s mission is to explore technological advancements..."
                  </div>
                </motion.div>
              )}
              {activeStep === 4 && (
                ...
                >
                  ...
                  <p className="text-[rgb(var(--text-primary))]">Synthesizing answer with GPT-3.5...</p>
                </motion.div>
              )}
              {activeStep === 5 && (
                <motion.div
...
function ArchitectureDiagram() {
  ...
  const components = [
    { id: 'frontend', title: 'Frontend', desc: 'React + Vite hosted on Netlify', icon: Cloud },
    { id: 'gateway', title: 'API Gateway', desc: 'AWS API Gateway for WebSocket & REST', icon: Server },
    { id: 'lambda', title: 'Compute', desc: 'AWS Lambda (Serverless Python)', icon: Cpu },
    { id: 'chroma', title: 'Vector DB', desc: 'ChromaDB running in Docker', icon: Database },
    { id: 's3', title: 'Storage', desc: 'AWS S3 for documents & audio', icon: FileText },
  ];
      ...
      <div className="space-y-4">
        {components.map((item) => (
          <motion.div
            ...
            whileHover={{ x: 10, backgroundColor: 'rgba(var(--bg-secondary), 0.5)' }}
          ...
          >
            <div className={`rounded-lg p-2 transition-colors ${
              hoveredComponent === item.id ? 'bg-[rgb(var(--accent))]/20 text-[rgb(var(--accent))]' : 'bg-[rgb(var(--bg-secondary))] text-[rgb(var(--text-secondary))]'
            }`}>
              <item.icon className="h-6 w-6" />
            </div>
            <div>
              <h4 className={`font-bold transition-colors ${hoveredComponent === item.id ? 'text-[rgb(var(--accent))]' : 'text-[rgb(var(--text-primary))]'}`}>
                {item.title}
              </h4>
  ...
  return (
    <motion.div
      animate={{
        ...
        borderColor: isActive ? 'rgba(var(--accent), 1)' : 'rgba(var(--border), 1)',
        backgroundColor: isActive ? 'rgba(var(--bg-primary), 1)' : 'rgba(var(--bg-secondary), 0.5)',
    ...
    >
      <div className={`rounded-full p-2 ${isActive ? 'text-[rgb(var(--accent))] bg-[rgb(var(--accent))]/10' : 'text-[rgb(var(--text-secondary))]'}`}>
        <Icon className="h-6 w-6" />
      </div>
      <span className={`text-sm font-bold ${isActive ? 'text-[rgb(var(--accent))]' : 'text-[rgb(var(--text-primary))]'}`}>
        {label}
      </span>
    </motion.div>
  );
